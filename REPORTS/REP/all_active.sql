...
select ab.filial,
       ...
       'Интернет '||reports.rpt_p_$utils.get_abon_addparam_value(ab.id, 44, '') as srv_main
 from (select distinct (a.id) id, 
      ...
      (select dts.column_value as dt from table(reports.rpt_p_$utils.get_dates(to_date('31.07.2015','dd.mm.yyyy'), to_date('31.07.2015','dd.mm.yyyy'))) dts) dates
