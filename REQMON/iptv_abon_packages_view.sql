...
select to_char(ab.card_num)            abon_num,
 ...
 where ab.id = srv.abonent_id
   and (srv.type_id = 53 -- Àáîíïëàòà
    and srv.plan_id in (2900, 2901, 2902) -- ÒÏ ïàêåòîâ IPTV
    and srv.state_id = 100003 -- Óñëóãà " Â ğàáîòå"
    and (srv.bdate <= sysdate and (srv.edate is null or srv.edate >= sysdate)))
union
select ap.pin,
       case
         when instr(trim(upper(ap.package)),'AMEDIA')   > 0 then 500
         when instr(trim(upper(ap.package)),'ÂÇĞÎÑËÛÉ') > 0 then 501
         when instr(trim(upper(ap.package)),'ÔÓÒÁÎË')   > 0 then 499
         else -1
       end package 
  from lt_abon_packages$ ap
 where (trim(upper(ap.package)) like '%AMEDIA%')
    or (trim(upper(ap.package)) like '%ÂÇĞÎÑËÛÉ%')
    or (trim(upper(ap.package)) like '%ÔÓÒÁÎË%');
